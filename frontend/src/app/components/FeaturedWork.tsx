...
const FeaturedWork = () => {
  ...
  return (
    <>
      ...
      <section className="hidden lg:block p-4 h-screen">
        <div className="bg-black rounded-2xl h-full flex justify-between">
          <div className="flex-1 py-16 px-8 space-y-20 text-white">
            <h1 className="text-lg font-medium">Featured Work</h1>
            <div className="h-[65vh] overflow-scroll">
              {works?.map((work) => (
                <div key={work.id}>
                  <h1 className="text-[82px] leading-none font-semibold cursor-pointer">
                    {work.title}
                  </h1>
                </div>
              ))}
          ...
          <div className="flex-1 p-8 pb-0 overflow-scroll">
              ...
              {works?.map((work) => (
                ...
                >
                  <img
                    src={work.image}
                    alt={work.title}
                    className="w-3xl object-cover transition-transform duration-500 group-hover:scale-105"
                  />
                  {/* Expanding circle background */}
                  <div
                    className="absolute -bottom-8 left-1/2 -translate-x-1/2 w-8 h-8 rounded-full scale-0 group-hover:scale-[150] origin-center transition-transform duration-700 ease-in-out z-10 pointer-events-none"
                    style={{ backgroundColor: work.color }}
                  ></div>

                  {/* Description text */}
                  <div className="absolute top-0 left-0 w-full p-8 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100 z-20 pointer-events-none">
